...
class CarModel():
    ...
    def __init__(self):
        ...
        self.mink1 = 43449.8  # Soft spring constant of suspension in N/m (more displacement under load)
        self.maxk1 = 86899.6  # Stiff spring constant of suspension in N/m (less displacement under load)
        self.mink2 = 3862.2  # Soft spring constant of tire in N/m (more displacement under load)
        self.maxk2 = 7724.4   # Stiff spring constant of tire in N/m (less displacement under load)
        self.accel =None
        self.accelMax = 19.62  # Maximum acceleration allowed in m/s^2 (~2g)
        self.accelLim = 19.62  # Acceleration limit in m/s^2 (~2g)
        self.SSE = 0.0
...
class CarView():
    ...
    def buildScene(self):
        ...
        self.SuspensionSpring = Spring(0, -20, pen=self.penSpring, name="Suspension Spring")
        self.TireSpring = Spring(0, 30, pen=self.penSpring, name="Tire Spring")
        self.Damper = Damper(0, 10, pen=self.penDamper, name="Damper")

        # Add to scene
        self.scene.addItem(self.SuspensionSpring)
        self.scene.addItem(self.TireSpring)
        self.scene.addItem(self.Damper)

        # Connect components (assuming methods exist in your Wheel and MassBlock classes)
        self.SuspensionSpring.connect(self.Wheel, self.CarBody)
        self.TireSpring.connect(self.Wheel, self.scene)  # assuming connection to a fixed point in scene
        self.Damper.connect(self.Wheel, self.CarBody)
...
class CarController():
    ...
    def ode_system(self, X, t):
        ...
        x2ddot = (self.model.k1 * (x1 - x2) + self.model.c1 * (x1dot - x2dot) - self.model.k2 * (
                    x2 - y)) / self.model.m2
    ...
    def calculate(self, doCalc=True):
        ...
        self.model.m1 = float(self.le_m1.text())  # Assuming 'le_m1' is the QLineEdit for mass of car body
        self.model.m2 = float(self.le_m2.text())  # QLineEdit for mass of wheel
        self.model.c1 = float(self.le_c1.text())  # QLineEdit for damping coefficient
        self.model.k1 = float(self.le_k1.text())  # QLineEdit for spring constant of suspension
        self.model.k2 = float(self.le_k2.text())  # QLineEdit for spring constant of tire
        self.model.v = float(self.le_v.text())  # QLineEdit for velocity of car

        # Recalculate min and max k values
        # These might be derived based on new model properties or set directly
        self.mink1 = float(self.le_mink1.text())  # Assuming there are fields to set or compute these
        self.maxk1 = float(self.le_maxk1.text())
        self.mink2 = float(self.le_mink2.text())
        self.maxk2 = float(self.le_maxk2.text())

        # This is the height of the ramp in m
        ymag = 6.0 / (12.0 * 3.3)
        if ymag is not None:
            self.model.ymag = ymag

        # Angle and maximum time set by user
        self.model.yangdeg = float(self.le_ang.text())  # QLineEdit for ramp angle
        self.model.tmax = float(self.le_tmax.text())  # QLineEdit for max time

        if doCalc:
            self.doCalc()  # Perform the calculation

        # Compute SSE, though typically you might want to check inputs or conditions
    ...
    def OptimizeSuspension(self):
        ...
        """
        # Step 1:
        # Set the initial parameters based on GUI inputs without recalculating everything
        self.calculate(doCalc=False)

        # Step 2:
        # Create an initial guess for k1, c1, and k2 based on either default values or last known good values
        x0 = np.array([self.model.k1, self.model.c1, self.model.k2])  # numpy array with initial values for k1, c1, k2

        # Step 3:
        # Use the Nelder-Mead method to minimize the SSE function (our objective function)
        # Assuming self.SSE is a callable that computes the sum of squared errors for given [k1, c1, k2]
        # and self.objFun is set to minimize this SSE
        result = minimize(self.SSE, x0, method='Nelder-Mead')

        # After optimization, update model parameters with optimized values
